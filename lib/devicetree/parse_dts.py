...
def write_special_props(node):
    ...
    for i, node_label in enumerate(node.labels):
        out_dt_define(f"{node.z_path_id}_MP_QSTR_IDX_{i}",
                      f"MP_QSTR_{node_label.upper()}")
    out_dt_define(f"{node.z_path_id}_MP_QSTR_NUM", len(node.labels))
    out_dt_define(f"{node.z_path_id}_MP_ROM_PTR", f"&pin_{node.label}")
